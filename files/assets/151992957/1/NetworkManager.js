...
BikeObby_NetworkManager.prototype.initialize = async function () {
    ...
    this.mobileControls.enabled = this.app.touch != null;
    ...
    this.stagesParent = this.app.root.findByName("Stages");
    this.stageUIText = this.app.root.findByName("StageUIText");
    this.coinText = this.app.root.findByName("CoinText");
    this.endConfetti = this.app.root.findByName("EndConfetti").particlesystem;
    ...
    this.mainMenuButton.button.on('click', function (event) {
    ...
    this.discordButton.button.on('click', async function (event) {
        window.open("https://discord.gg/WydRVbDkcJ");
    }, this);

    this.menuButtonClose = this.menuPanel.findByName("CloseButton");
    this.menuButton.children[0].enabled = this.app.touch == null;
    ...
    this.shopButton.children[0].enabled = this.app.touch == null;
    ...
    this.resetButton.children[0].enabled = this.app.touch == null;
...
BikeObby_NetworkManager.prototype.update = function (dt) {
    ...
    if (this.app.firstTouch == true && this.app.canLockPointer == true && this.mouseLockCounter > 10) {
        ...
        if (!this.app.touch && pc.Mouse.isPointerLocked() === false && camResult === false) {
...
BikeObby_NetworkManager.prototype.joinServer = async function () {
    try {
        ...
        this.app.colyseus = new Colyseus.Client("wss://obby.tufapps.com");
        ...
        if (BikeObby_Utils.getItem("BIKEOBBY_createCustomRoom") === "1") {
            ...
            this.app.fire("popupController:showPopup",
                "Private Room Created", "Room code: " + this.room.roomId, false, this, "copiedRoomId", "closePopup", { yesButtonText: "Copy" });
...
BikeObby_NetworkManager.prototype.increaseStage = function () {
    ...
    this.stagePercentageText.text = this.stageUIText.element.text + "%";
    this.stagePercentagePin.setLocalPosition(this.app.currentStage * 5.37, -40, 0);
