...
BikeObby_PlayerController.prototype.respawn = function () {
    ...
    PokiSDK.commercialBreak(() => {
        ...
        this.ball.rigidbody.applyImpulse(this.networkManager.stagesParent.children[this.app.currentStage].forward.scale(3));
...
BikeObby_PlayerController.prototype.setRefRotation = function (dt) {
    ...
    if (this.app.touch) { // Mobile
        ...
        if (x === 0 && z === 0) {
            ...
            this.refPointForRotation.setRotation(playerRot);
        }
    } else { // PC
        if (this.app.keyboard.isPressed(pc.KEY_A) || this.app.keyboard.isPressed(pc.KEY_LEFT)) {
            x = 1;
...
BikeObby_PlayerController.prototype.lerpBikeToBall = function (dt) {
    ...
    if (distance < dt) {
        this.interpolatedPosition.lerp(this.bike.getPosition(),
            this.ball.getPosition(), dt);
    } else {
        this.interpolatedPosition.lerp(this.bike.getPosition(),
            this.ball.getPosition(), 15 * dt);
    }
    ...
    this.bike.setRotation(this.moveDirectionOrginal);
